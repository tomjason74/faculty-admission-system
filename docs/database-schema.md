...
## Decisions Made
1. **Faculty Profile Fields**: The `faculty_profiles` table will include: `user_id` (FK), `degree` (String), `specialization` (String), `department_id` (FK), `employment_type` (Enum), `hire_date` (Date, nullable), and `status` (Enum).
